...
def db_city(name,pais,Nopo,Nohou,Nosto):
  
  #crea el cliente donde se va a almacenar la informacion
  db = client.dbcity_and_sto
  
  #crea la base a la que se le va a ingresar informacion
  dbcity = db["cities"]
  
  #estructura la informacion segun la base de datos
  new_city = [{"name":name , "country":pais , "poblation":Nopo , "recidence":Nohou , "stores":Nosto}]
  
  #intento de almacenar la informacion
  try: 
    result = dbcity.insert_many(new_city)
    print(result)
